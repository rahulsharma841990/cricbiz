...
export class TournamentCreatePage implements OnInit {
	...
	public formData = {
		...
		orgnizerName: '',
		orgnizerNumber: '',
		AllowContactOnWhatsapp: '',
		startDate: '',
		endDate: '',
		tournament_categor_id: '',
		ballType: '',
		numberOfOvers: '',
		tags: '',
		moreDetails: '',
	...
	ngOnInit() {
		this.utilService.observeMe().subscribe((type) => {
			if (type.type == 'city') {
				...
				this.formData['city_id'] = localStorage.getItem('city_id');
			}
		});
		this.utilService.presentLoadingWithOptions().then(() => {
			this.api.request('tournamentCategories').then((res: any) => {
	...
	public setCategory(category) {
		this.formData['tournament_categor_id'] = category.tournament_categor_id;
	}
